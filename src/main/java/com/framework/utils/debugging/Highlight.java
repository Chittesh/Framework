package com.framework.utils.debugging;
...
import com.framework.utils.Sleeper;
...
public class Highlight {
    ...
    public static void flashHighlightSuccess(WebDriver driver, WebElement element) throws InterruptedException {
        highlight(driver, element, jsHighlight, defaultSuccessColor);
        Sleeper.sleep(.5);
    ...
    private static void highlight(WebDriver driver, WebElement element, String js, Colors color) {
        ...
        try {
            if (driver instanceof AllegisDriver) {
                ((AllegisDriver) driver).executeJavaScript(js, element);
            } else {
